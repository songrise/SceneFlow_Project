...
pc1 = []
pc2 = []
# generate a mesh data (two planes)
for x in range(0,100):
    for y in range(0, 100):
        pc1.append([x/10,y/10,0.0])
        if np.random.uniform(0,1)>=drop_prob:
            pc2.append([x/10,y/10+1.0,1.0])

gt = [[0.0,1.0,1.0] for _ in range(10000)]

pc1 = np.array(pc1,dtype=float)
pc2 = np.array(pc2,dtype=float)
gt = np.array(gt,dtype = float)
print("pc1 shape{} pc2 shape{} gt shape {}".format(pc1.shape,pc2.shape,gt.shape))
...
if True:
    ...
    with tf.Graph().as_default():
        ...
        for i in range(0, len_cloud, 2048):
            if i + 2048 < len(pc1) and i + 2048 < len(pc2):

                pc1_ = pc1[i:i + 2048, :3]
                pc2_ = pc2[i:i + 2048, :3]
                gt_ = gt[i:i + 2048, :3]
                pc1_ = pc1_ - ref_center
                pc2_ = pc2_ - ref_center
                batch_data.append(np.concatenate([np.concatenate([pc1_,
                                                                  pc2_], axis=0),
                                                  np.zeros((4096, 3))], axis=1))  # 4096, 6
                batch_label.append(gt_)
        ...
        gt_flow = pred_val[0] + pc1_  # ! predicted second frame

        pcd1 = o3d.geometry.PointCloud()
        pcd1.points = o3d.utility.Vector3dVector(pc1_)
        pcd1.paint_uniform_color([1, 0, 0])  # ! Re: red

        pcd2 = o3d.geometry.PointCloud()
        pcd2.points = o3d.utility.Vector3dVector(pc2_)
        pcd2.paint_uniform_color([0, 1, 0])  # ! Re: green
        ...
        # np.savetxt("pred_val.csv",pred_val[0])
