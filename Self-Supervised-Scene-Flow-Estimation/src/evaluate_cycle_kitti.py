...
parser.add_argument('--model_path', default='log_train/model.ckpt',
...
def scene_flow_EPE_np(pred, labels, mask):
    #!Re end point error., this is metric, which mathematically equivalent with
    #! the supervised L2 loss (see paper)
    #! notice that both pred and labels are flow (i.e. translational vector),
    #! instead of the points. hence the `pred-labels` is also vectors.

    error = np.sqrt(np.sum((pred - labels)**2, 2) + 1e-20)
    #! `mask` is a boolean list indicating which sample points shall be
    #! invovled in cal of accuracy.
    #! the reason behind is that `pred` is just a subset of `lables`.
    #! another reason is (perhaps) two frames has inconsistent number of points
    #! the annotation made by author are reminding that 2nd axis is each sample.
    gtflow_len = np.sqrt(np.sum(labels*labels, 2) + 1e-20)  # B,N
    #!Re : ACC_0.05 and ACC_0.1 respectively
    ...
    #!Re: effectively length of `pred` (see usage)
    mask_sum = np.sum(mask, 1)
    acc1 = acc1[mask_sum > 0] / mask_sum[mask_sum > 0]
    acc1 = np.sum(acc1)
    acc2 = acc2[mask_sum > 0] / mask_sum[mask_sum > 0]
    acc2 = np.sum(acc2)
    #! Re: does not divied by N. because it will be done later,,
    EPE = np.sum(error)
...
#     all_kitti = glob.glob('/home/gaurav/himangi/flownet3d_research/cvpr/after_cvpr/rebuttal/kitti_final_points_estimate/*.npz')
    ...
    for ki in all_kitti:
        ...
        for i in range(0, len_cloud, 2048):
            #! Re evaluate each scene with a stride of 2048.
            #! a scene has some 30000 points, and it is evaluating partly in one run.
            #! see visualization.py for a demo
        ...
        pred_val, end_points_val = sess.run(
            [ops['pred'], ops['end_points']], feed_dict=feed_dict)
    ...
    print('Num batches {} Average EPE {}'.format(
        sample_count, epe_total/sample_count))
